...
def main():
    ...
    # Video Params.
    interpolation = config_dict["interpolation"]
    ...
    logging.info(f"Video Frame Interpolation: {interpolation}")
    ...
    for epoch in range(starting_epoch, max_epoch):
        ...
        for index, video_frames in enumerate(dataloader):
            ...
            N, C, Frames, H, W = video_frames.shape
            ...
            if global_steps % checkpoint_steps == 0 and global_steps >= 0:
                ...
                # X_T ~ N(0, I).
                noise = torch.randn((plot_img_count, C, Frames, H, W), device=device)
                ...
                N, C, Frames, H, W = x0_approx.shape
                x0_approx = x0_approx.permute(0, 1, 3, 2, 4).reshape(N, C, H, Frames*W)
                # x0_approx = x0_approx.permute(0, 2, 1, 3, 4).reshape(N*F, C, H, W)
